...
def main():
    ...
    server = socket(AF_INET, SOCK_DGRAM) #Initial Socket interface.
    server.bind(('',port))
    ...
    while True:
        try:
            received , clientAddress = server.recvfrom(2048)
            visitID = received.decode().split()[1]
            ...
            server.sendto(msg.encode(), clientAddress)
